...
@movie_bp.route("/filme/<int:movie_id>", methods=["GET"])
def buscar_filme(movie_id):
    conn = get_connection()
    cur = conn.cursor()
...
def criar_filme():
    ...
    try:
        cur.execute("""
            INSERT INTO filme (titulo, diretor, dt_lancamento, sinopse, capa_filme)
            VALUES (%s, %s, %s, %s, %s)
            RETURNING id
        """, (titulo, diretor, dt_lancamento, sinopse, capa_filme))
...
def modificar_filme(movie_id):
    ...
    campos = ["titulo", "diretor", "dt_lancamento", "sinopse", "capa_filme"]
