...
def deletar_usuario(user_id):
    ...
    admin_deletando = data.get("admin_deletando") if data else False  

    conn = get_connection()
    cur = conn.cursor()

    try:
        cur.execute("SELECT senha_hash FROM adm WHERE id = %s", (user_id,))
        admin = cur.fetchone()

        if admin:
            if not admin_deletando:  
                if not senha:
            ...
            cur.execute("DELETE FROM adm WHERE id = %s", (user_id,))
            conn.commit()
            return jsonify({"status": "Admin deletado"}), 200

        cur.execute("SELECT senha_hash FROM usuario WHERE id = %s", (user_id,))
        ...
        cur.execute("DELETE FROM favorito WHERE usuario_id = %s", (user_id,))
        cur.execute("DELETE FROM assistido WHERE usuario_id = %s", (user_id,))
        cur.execute("DELETE FROM usuario WHERE id = %s", (user_id,))
        
        conn.commit()
        
        return jsonify({"status": "Usuário deletado"}), 200
