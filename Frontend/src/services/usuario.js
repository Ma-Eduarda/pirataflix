...
export const deletarUsuario = async (userId, senha = null, adminDeletandoOutroUsuario = false) => {
    const body = {};
    ...
    if (adminDeletandoOutroUsuario) {
        body.admin_deletando = true;  
    }

    const response = await api.delete(`/usuarios/${userId}`, { data: body }); 
...
// ASSISTIDOS 
export async function listarAssistidos(id_usuario) {
    const resposta = await api.get(`/usuarios/${id_usuario}/assistidos`);
    return resposta.data;
}
